...
namespace adt
{
    ...
    public:
        struct node
        {
            ...
            node(const type &val, const node &next_) : value(new type(val)), next(new node(next_)) {}
            ~node()
            {
                delete next;
                delete value;
        ...
        explicit linkedlist();
        ~linkedlist();
        explicit linkedlist(const node &node_);
...
T linkedlist<type>::linkedlist(const node &node_) : head(new node(node_)) {} // node(2, node(5,node(2)))
